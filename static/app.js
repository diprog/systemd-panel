...
let logSource = null;
let currentLogMode = "short";   // "short" | "cat"
...
function setLogsMode(mode) {
    currentLogMode = mode === "cat" ? "cat" : "short";
    const wrap = $("#logs-mode");
    if (wrap) {
        const btns = wrap.querySelectorAll("button[data-mode]");
        btns.forEach((b) => {
            const isActive = b.getAttribute("data-mode") === currentLogMode;
            if (isActive) {
                b.classList.add("bg-brand-500", "text-white");
                b.classList.remove("hover:bg-white/90", "dark:hover:bg-neutral-900/90");
            } else {
                b.classList.remove("bg-brand-500", "text-white");
                b.classList.add("hover:bg-white/90", "dark:hover:bg-neutral-900/90");
            }
        });
    }
}
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const logsModeWrap = $("#logs-mode");
    if (logsModeWrap) {
        logsModeWrap.addEventListener("click", (e) => {
            const btn = e.target.closest("button[data-mode]");
            if (!btn) return;
            const next = btn.getAttribute("data-mode");
            if (next === currentLogMode) return;
            setLogsMode(next);
            const pre = $("#logs-pre");
